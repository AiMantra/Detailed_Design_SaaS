...
const ProjectReport = () => {
    ...
    const reportData = useMemo(() => {
        ...
        const processedProjects = projects.map((project) => {
            ...
            const processedActivities = (project.activities || []).map((activity) => {
                ...
                const processedSubactivities = (activity.subactivities || []).map(
                    (sub) => {
                        let subTotalSeconds = 0;
                        const usersList = [];
                        const allLogs = [];

                        (sub.users || []).forEach((user) => {
                            const userSeconds = timeToSeconds(user.total_time_spent);
                            ...
                            globalEmp.total_hours_in_seconds += userSeconds;
                            globalEmp.total_tasks++;
                            globalEmp.projects.add(project.project_id);
                            globalEmp.statusBreakdown[sub.status] =
                                (globalEmp.statusBreakdown[sub.status] || 0) + 1;
                            ...
                            projectEmp.total_hours_in_seconds += userSeconds;
                            ...
                            usersList.push({
                                ...user,
                                hours_in_seconds: userSeconds,
                                hours_formatted: formatSecondsToDuration(userSeconds),
                                logs: user.date_wise,
                            });
                        });
                        ...
                        return {
                            ...
                            total_hours_in_seconds: subTotalSeconds,
                            total_hours_formatted: formatSecondsToDuration(subTotalSeconds),
                            users: usersList,
                            logs: allLogs,
            ...
            return {
                ...
                total_hours_in_seconds: projectTotalSeconds,
                total_hours_formatted: formatSecondsToDuration(projectTotalSeconds),
                ...
                employees: Array.from(projectEmployeesMap.values())
                    .map((emp) => ({
                        ...emp,
                        total_hours: formatSecondsToDuration(emp.total_hours_in_seconds),
                        total_hours_in_seconds: emp.total_hours_in_seconds,
                    }))
                    .sort((a, b) => b.total_hours_in_seconds - a.total_hours_in_seconds),
            };
        });
        ...
        return {
            ...
            totalStats: {
                ...
                totalHoursFormatted: formatSecondsToDuration(totalHoursInSeconds),
    ...
    const filteredData = useMemo(() => {
        if (!reportData.projects.length)
            return { projects: [], filteredStats: reportData.totalStats };
            ...
            filteredProjects = filteredProjects
                .map((project) => ({
                    ...
                    activities: project.activities
                        .map((activity) => ({
                            ...
                            subactivities: activity.subactivities
                                .map((sub) => ({
                                    ...sub,
                                    users: sub.users.filter(
                                        (user) => user.emp_code === selectedEmployee,
                                    ),
                                    logs: sub.logs.filter(
                                        (log) => log.user_code === selectedEmployee,
                                    ),
                                }))
                    ...
                    employees: project.employees.filter(
                        (emp) => emp.emp_code === selectedEmployee,
                    ),
                }))
                .filter((project) => project.activities.length > 0);
        }
            ...
            filteredStats: {
                ...
                totalEmployees:
                    selectedEmployee !== "all"
                        ? filteredProjects.reduce((sum, p) => sum + p.employees.length, 0)
                        : reportData.totalStats.totalEmployees,
                totalHoursInSeconds: filteredTotalSeconds,
                totalHoursFormatted: formatSecondsToDuration(filteredTotalSeconds),
                    ...
                    <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 rounded-xl">
                        <div>
                            ...
                            <p className="text-purple-100 text-lg">
                                Complete overview of all projects, activities, and team
                                performance metrics
                            </p>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-2 mt-6">
                            ...
                            {selectedEmployee !== "all" && (
                                <span className="inline-flex items-center gap-2 bg-white/20 backdrop-blur-sm px-3 py-1.5 rounded-full text-sm">
                                    <User size={14} />
                                    {
                                        reportData.allEmployees.find(
                                            (e) => e.emp_code === selectedEmployee,
                                        )?.name
                                    }
                                    <button
            ...
            <div className="max-w-full mx-auto px-6 py-6">
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-xl font-bold text-purple-700">
                                    {hasActiveFilters
                                        ? filteredData.filteredStats.totalProjects
                                        : reportData.totalStats.totalProjects}
                                </p>
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-xl font-bold text-blue-700">
                                    {hasActiveFilters
                                        ? filteredData.filteredStats.totalEmployees
                                        : reportData.totalStats.totalEmployees}
                                </p>
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-xl font-bold text-green-700">
                                    {hasActiveFilters
                                        ? filteredData.filteredStats.totalHoursFormatted
                                        : reportData.totalStats.totalHoursFormatted}
                                </p>
                ...
                <div className="mb-4 flex justify-between items-center">
                    <p className="text-sm text-gray-600">
                        ...
                        of{" "}
                        <span className="font-semibold">{reportData.projects.length}</span>{" "}
                        projects
                        {hasActiveFilters && (
                            <span className="text-purple-500 ml-2">(filtered results)</span>
                        )}
                    </p>
                </div>
                ...
                <div className="space-y-4">
                    {filteredData.projects.map((project, index) => {
                        ...
                        const completionRate = (
                            (project.statusBreakdown.Approved / project.total_tasks) *
                            100
                        ).toFixed(0);
                        ...
                        return (
                            ...
                            >
                                ...
                                <div
                                    ...
                                    onClick={() =>
                                        setExpandedProject(
                                            isProjectExpanded ? null : project.project_id,
                                        )
                                    }
                                >
                                            ...
                                            <div className="flex items-center gap-3 mb-2 flex-wrap">
                                                ...
                                                <div className="flex gap-2">
                                                    <span className="text-xs text-gray-500 flex items-center gap-1">
                                                        <Activity size={12} /> {project.total_activities}{" "}
                                                        activities
                                                    </span>
                                                    <span className="text-xs text-gray-500 flex items-center gap-1">
                                                        <Users size={12} /> {project.total_employees}{" "}
                                                        members
                                                    </span>
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex gap-3">
                                                ...
                                                {project.statusBreakdown.Approved > 0 && (
                                                    <div className="text-center">
                                                        ...
                                                        <p className="text-xs font-medium text-gray-600 mt-1">
                                                            {project.statusBreakdown.Approved}
                                                        </p>
                                                    </div>
                                                )}
